    ...
    $.mobiscroll.classes.Scroller = function (el, settings) {
            ...
            $.each(values, function (j, v) {
                ...
                if (keys[j] < 12) {
                    html += '<div role="option" aria-selected="false" class="dw-li dw-v" data-val="' + keys[j] + '"' + (labels[j] ? ' aria-label="' + labels[j] + '"' : '') + ' style="height:' + itemHeight + 'px;line-height:' + itemHeight + 'px;">' +
                        '<div class="dw-i"' + (lines > 1 ? ' style="line-height:' + Math.round(itemHeight / lines) + 'px;font-size:' + Math.round(itemHeight / lines * 0.8) + 'px;"' : '') + '>' + '<span class="dw-mon">' + s.monthNamesShort[j] + '</span>' + v + '</div></div>';
                    l++;
                }
                else {
                    html += '<div role="option" aria-selected="false" class="dw-li dw-v" data-val="' + keys[j] + '"' + (labels[j] ? ' aria-label="' + labels[j] + '"' : '') + ' style="height:' + itemHeight + 'px;line-height:' + itemHeight + 'px;">' +
        ...
        */
        that.init = function (ss) {
            ...
            that.buttons.set = { text: s.setText, css: 'dwb-s-text', handler: that.select };
            that.buttons.cancel = { text: (that.live) ? s.closeText :"", css: 'dwb-c', handler: that.cancel };
            that.buttons.clear = {
                text: s.clearText,
                css: 'dwb-cl',
                handler: function () {
